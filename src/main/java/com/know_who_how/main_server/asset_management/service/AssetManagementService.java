...
public class AssetManagementService {
        ...
        public SimulationResponse runDepositSimulation(SimulationRequest request) {
                ...
                BigDecimal interestRate = getInterestRateForDeposit(product.getInterestRateDetails(),
                                request.periodMonths());
        ...
        private BigDecimal getInterestRateForDeposit(String interestRateDetails, int months) {
                ...
                try {
                        ...
                        return tiers.tiers().stream()
                                        ...
                                        .orElseThrow(() -> new CustomException(ErrorCode.INTERNAL_SERVER_ERROR)); // Or
                                                                                                                  // rate
                                                                                                                  // not
                                                                                                                  // found
                                                                                                                  // for
        ...
        private PortfolioResponse.CashFlowDto createCashFlowDto(boolean isSavingsType, long monthlyNetSavings,
                ...
                if (isSavingsType) {
                        return new PortfolioResponse.CashFlowDto("월 저축형", monthlyNetSavings, null,
                                        product.getProductName(), interestRate);
                } else {
                        return new PortfolioResponse.CashFlowDto("목돈 예치형", null, idleCashAssets,
                                        product.getProductName(), interestRate);
                }
        }
        ...
        private PortfolioResponse.PredictionDto createPredictionDto(boolean isSavingsType, long idleCashAssets,
                ...
                if (isSavingsType) {
                        ...
                        Map<String, Long> result = calculateDeposit(principal,
                                        getInterestRateForDeposit(product.getInterestRateDetails(),
        ...
        private Map<String, Long> calculateInstallmentSavings(long monthlyDeposit, BigDecimal interestRate,
                        int months) {
                ...
                BigDecimal interest = md.multiply(monthlyRate)
                                .multiply(m.multiply(m.add(BigDecimal.ONE)).divide(BigDecimal.valueOf(2), 10,
