...
public class AssetManagementService {
    ...
    public PortfolioResponse getPortfolio(User user) {
        ...
        // 총자산 계산 및 업데이트 (자산 - 대출)
        calculateAndUpdateAssetTotal(managedUser);
    ...
    @Transactional
    public void calculateAndUpdateAssetTotal(User user) {
        List<Asset> assetsList = assetsRepository.findByUser(user);
        long totalAsset = assetsList.stream()
                .mapToLong(asset -> {
                    if (asset.getType() == AssetType.LOAN) {
                        return -asset.getBalance().longValue();
                    }
                    return asset.getBalance().longValue();
                })
                .sum();
        user.updateAssetTotal(totalAsset);
